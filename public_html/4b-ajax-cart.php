...
switch ($_POST['req']) {
  ...
  case "show":
    ...
    <form id="cart-checkout" onsubmit="return cart.checkout();">
      <label>Name:</label>
      <input type="text" id="co_name" placeholder="Nathan Bayne" required/>
      <label>Email:</label>
      <input type="email" id="co_email" placeholder="nathan.bayne@yahoo.com" required/>
  ...
  case "checkout-email":
    ...
    if ($cartLib->checkout($_POST['name'], $_POST['email'])) {
      ...
      $message = "You have purchased your tickets successfully! Your tickets will be sent to you at a later date. We hope you enjoy your visit at our beautiful museum!";
      foreach ($order['items'] as $pid=>$p) {
        $message .= $p['e_name'] . " - " . $p['quantity'] . "<br>";
      }
      $headers = implode("\r\n", [
        'MIME-Version: 1.0',
